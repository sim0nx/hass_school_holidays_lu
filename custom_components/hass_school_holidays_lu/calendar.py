...
async def async_setup_entry(
    ...
    async_add_entities(
        [CustomCalendarEntity(hass, event_url, entity_name, preferred_lang)]
    )
    _LOGGER.info(
        f"CustomUrlCalendar successfully set up from config entry for URL: {event_url}"
    )
    return True
...
class CustomCalendarEntity(CalendarEntity):
    ...
    def __init__(
        self, hass: HomeAssistant, event_url: str, name: str, preferred_lang: str
    ):
    ...
    async def _async_load_events_from_file(self, filename: str):
        ...
        component_dir = os.path.join(
            hass.config.config_dir, "custom_components", DOMAIN
        )
    ...
    async def async_get_events(
        ...
        """
        _LOGGER.debug(
            f"Fetching events from {self._event_url} between {start_date} and {end_date}"
        )
        ...
        try:
            async with session.get(self._event_url, timeout=10) as response:
                if response.status == 200:
                    ...
                    _LOGGER.warning(
                        f"URL fetch failed (Status: {response.status}). HTTP error encountered."
                    )

        except Exception as err:
            _LOGGER.error(
                f"Network error fetching data from URL: {err}. Proceeding to fallback."
            )
        ...
        if raw_events is None:
            _LOGGER.error(
                "Failed to retrieve events from both URL and fallback file. Returning empty list."
            )
            return []
        ...
        def _get_localized_summary(
            localized_summary_data, preferred_lang, fallback_lang="en"
        ):
            ...
            if fallback_lang in localized_summary_data:
                _LOGGER.debug(
                    f"Title missing for {preferred_lang}, falling back to {fallback_lang}."
                )
            ...
            for lang in SUPPORTED_LANGUAGES:
                if lang in localized_summary_data:
                    _LOGGER.warning(
                        f"Title missing for {preferred_lang} and EN, using available language {lang}."
                    )
        ...
        for raw_event in raw_events:
            try:
                ...
                if not all([summary, start_dt_str, end_dt_str]):
                    _LOGGER.warning(
                        f"Skipping malformed event (missing summary, start, or end): {raw_event}"
                    )
                    continue
